    ...
    <Card key={post.id} className="mb-6">
      <CardHeader className="flex flex-row items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">by {poster?.name}</p>
        </div>
      </CardHeader>
      <CardContent>
        <p>{post.content}</p>
      ...
      <CardFooter className="flex justify-between">
          ...
          >
            <ArrowBigUp
              className={`mr-1 h-4 w-4 ${
                user && user.votes[post.id] && user.votes[post.id].type == 1
                  ? "fill-current"
                  : ""
              }`}
            />
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Bookmark
              className={`mr-1 h-4 w-4 ${
                user && user?.saved.includes(post.id) ? "fill-current" : ""
              }`}
            />
            {user && user?.saved.includes(post.id) ? "Saved" : "Save"}
          </Button>
          <Button
            variant="outline"
            size="sm"
